...
class InViewObserver {
  constructor(options = {}) {
    const {
      selector = ".m-inView",
      // ターゲットセレクタ
      stateClass = "js-inView",
      // 付与するクラス名
      offset = 0,
      // 追加発火を早めるための下端オフセット(px)
      exitOffset = 0,
      // ビューポートから離れる際のオフセット(px)
      removeOnExit = false
      // falseでビューポート外で state を外す
    } = options;
...
const _Modal = class _Modal {
  constructor(options = {}) {
    const cfg = { ..._Modal.DEFAULTS, ...options };
    this.cfg = cfg;
    this.mode = cfg.mode;
  ...
  destroy() {
    ...
    if ((_a = this.modal) == null ? void 0 : _a.parentNode) this.modal.parentNode.removeChild(this.modal);
  }
};
