...
*/
var InViewObserver = class {
	constructor(options = {}) {
		const { selector = ".m-inView", stateClass = "js-inView", offset = 0, exitOffset = 0, removeOnExit = false } = options;
	...
	init() {
		...
		if ("IntersectionObserver" in window) this.setupIntersectionObserver();
	...
	setupIntersectionObserver() {
		this.enterIO = new IntersectionObserver((entries) => {
			for (const entry of entries) if (entry.isIntersecting) {
				...
				if (!this.removeOnExit) this.enterIO.unobserve(entry.target);
			}
		}, {
			root: null,
		...
		if (this.removeOnExit) {
			this.exitIO = new IntersectionObserver((entries) => {
				for (const entry of entries) if (!entry.isIntersecting) this.removeState(entry.target);
			}, {
				root: null,
		...
		} else this.targets.forEach((el) => this.enterIO.observe(el));
	}
	setupFallback() {
		...
		if (this.removeOnExit) {
			this.check = () => {
				for (const el of this.targets) {
					...
					if (isInViewportForEnter(rect)) this.addState(el);
					else if (!isInViewportForExit(rect)) this.removeState(el);
				}
			};
...
var PostIndex = class {
	...
	*/
	...
	_buildOne(tocRoot, searchRoot) {
		if (searchRoot.querySelectorAll("h2").length < this.settings.minH2ForDisplay) {
	...
	_getOffset() {
		const v = typeof this.settings.scrollOffset === "function" ? this.settings.scrollOffset() : this.settings.scrollOffset;
		return Number(v) || 0;
	}
	_slugify(text) {
		return String(text || "").trim().toLowerCase().replace(/[\s　]+/g, "-").replace(/[^\w\-ぁ-んァ-ン一-龥]/g, "").replace(/\-+/g, "-").replace(/^\-|\-$/g, "");
	}
	...
	_classFromSelector(sel) {
		...
		if (!sel.match(/^(\.[A-Za-z0-9\-\_\.]+)$/)) return "";
		return sel.split(".").filter(Boolean).join(" ");
	}
};
//#endregion
...
var Modal = class Modal {
	...
	async _show(index) {
		...
		if (item.type !== "video") this._preloadAround(items, index);
	}
	_getCurrentItems() {
...
function aside() {
	...
	if (toggleBtn && spNav) toggleBtn.addEventListener("click", () => {
	...
	if (spNav) {
		...
		window.addEventListener("scroll", () => {
			...
			if (Math.abs(y - lastY) >= SCROLL_OFFSET_PX) {
...
function smoothScroll(options = {}) {
	const { offset = 0, duration = 500, updateURL = "replace" } = options;
	const resolveOffset = () => {
	...
	const getY = (el) => {
		const rect = el.getBoundingClientRect();
		const y = (window.pageYOffset || root.scrollTop || 0) + rect.top - resolveOffset();
	...
	const isSamePageLink = (a) => a && a.pathname === location.pathname && a.origin === location.origin;
	document.addEventListener("click", (e) => {
		const a = e.target.closest("a[href^=\"#\"]");
		if (!a) return;
		const trimmed = (a.getAttribute("href") || "").trim();
...
function globalMenuToggle() {
	...
	nav.addEventListener("click", (ev) => {
		if (ev.target.closest("a[href^=\"#\"]")) close(ev);
	});
}
//#endregion
...
*/
function headerScroll(scrollAmount, selector = ".l-header", className = "js-scroll", offset = 5) {
	...
	window.addEventListener("scroll", () => {
		...
		if (currentScrollY >= scrollAmount) element.classList.add(className);
		else element.classList.remove(className);
		if (currentScrollY <= 80) element.classList.remove(hiddenClass);
		else if (currentScrollY > lastScrollY && currentScrollY > 0) element.classList.add(hiddenClass);
		else if (currentScrollY < lastScrollY - offset) element.classList.remove(hiddenClass);
